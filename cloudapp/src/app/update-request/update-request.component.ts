...
import {
    ...
    Entity, PageInfo, RestErrorResponse, EntityType
...
/*
    formGroup = new FormGroup({
            ...
            mmsId: new FormControl(this.mms_id),
        }
    )
*/

    constructor(private restService: CloudAppRestService, private appService: AppService, private eventsService: CloudAppEventsService, private toastr: ToastrService) { }

    ngOnInit() {
        this.appService.setTitle('Update Request-data')
    ...
    onPageLoad = (pageInfo: PageInfo) => {
        ...
        if ((pageInfo.entities || []).length == 1) {
            ...
            if( entity) {
                console.log("Entity as JSON: " + JSON.stringify(entity));
                let link = entity.link;
                if(link.includes('mms-id')) {
                    console.log("MMS-id findes");
                }
            }
